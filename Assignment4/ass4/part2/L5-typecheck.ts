...
import { isAppExp, isBoolExp, isDefineExp, isIfExp, isLetrecExp, isLetExp, isNumExp,
         ...
         Parsed, PrimOp, ProcExp, Program, StrExp, isLetValuesExp, LetValuesExp, CExp, Values } from "./L5-ast";
...
export const typeofApp = (app: AppExp, tenv: TEnv): Result<TExp> => {
    if (isPrimOp(app.rator) && app.rator.op === "values") { 
        return typeValues(app.rands, tenv);
    }
...
const typeValues = (rands: CExp[], tenv: TEnv): Result<TExp> => {
    const RandsType = mapResult((operand: CExp) => typeofExp(operand, tenv), rands);
    const unparsedRands = (texps: TExp[]): Result<string[]> => 
        mapResult((tExp: TExp) => unparseTExp(tExp), texps); 
    const unparsedRandsType = bind(RandsType, unparsedRands);
    const joinStrings = (texps: string[]) => makeOk(`(${join(" * ", texps)})`)
    const joinedStrings = bind(unparsedRandsType, joinStrings);
    return bind(joinedStrings, parseTE);
}

export const typeofLetvalues = (exp: LetValuesExp, tenv: TEnv): Result<TExp> => {
    const Vals = map((bindings: Values) => bindings.val, exp.bindings);
    const TExpsOfVals = mapResult((val: CExp) => typeofExp(val, tenv), Vals);
    const AllTuples = bind(TExpsOfVals, (texps: TExp[]): Result<TExp[][]> => 
            mapResult((t: TExp) => isTupleTExp(t) ? isEmptyTupleTExp(t) ? makeOk([]) : makeOk(t.TEs) : 
                makeFailure(`${unparseTExp(t)} is not a tuple`), texps));
    const Vars = map((b: Values) => b.vars, exp.bindings);
    const varTEss = map((vars) => map((v) => v.texp, vars), Vars);
    const checkEqual = bind(AllTuples, (vals: TExp[][]) => bind(mapResult((vars: TExp[]) => {
            const GoodVal = vals[varTEss.indexOf(vars)];
            return vars.length === GoodVal.length ? makeOk(true) :
            makeFailure(`Bad size ${vars} <-> ${vals}`);}, varTEss), _ => makeOk(vals)));
    const constraint = (rTEs: TExp[], lTEs: TExp[]) =>
        zipWithResult((varTE, valTE) => checkEqualType(varTE, valTE, exp), rTEs, lTEs);
    const Constraints = bind(checkEqual, (valTEss: TExp[][]) => mapResult((varTEs: TExp[]) => 
                constraint(varTEs, valTEss[varTEss.indexOf(varTEs)]), varTEss));
    const GoodVars = chain((x) => map((v: VarDecl) => v.var, x), Vars);
    const GoodVarTEs = chain((x) => x, varTEss);
    return bind(Constraints, _ => typeofExps(exp.body, makeExtendTEnv(GoodVars, GoodVarTEs, tenv)));
}