...
import { CExp, Exp, IfExp, LetrecExp, LetExp, ProcExp, Program, SetExp, isCExp, isLetValuesExp, LetValuesExp, Values } from './L5-ast';
...
const evalLetValues = (exp: LetValuesExp, env: Env): Result<Value> => {
    ...
    const evalVs = mapResult((exp: CExp) => applicativeEval(exp, env), Vals);
    const doTuple = (exp1: SExpValue): Result<SExpValue[]> => 
        isTuple(exp1) ? makeOk(exp1.vals) : makeFailure(`Bad tuple value: ${JSON.stringify(exp1)}`)
    const tupleVals = bind(evalVs, (vals: SExpValue[]) => mapResult(doTuple, vals))
    const Vars = makeOk(map((bindings: Values) => bindings.vars, exp.bindings));
    const isEqual = (l: VarDecl[], r: SExpValue[]): boolean => l.length === r.length;
    const checkEqual = (vars: VarDecl[][], vals: SExpValue[][]): Result<boolean[]> => 
        mapResult((l: VarDecl[]) =>
                    isEqual(l, vals[vars.indexOf(l)]) ? makeOk(true) : 
                    makeFailure(`Bad size: ${l} <-> ${vals[vars.indexOf(l)]}`), vars)
    const GoodVars = (vars: VarDecl[][]): string[] => 
        chain((x) => map((varsDecl: VarDecl) => varsDecl.var, x), vars);
    const GoodVals = (vals: SExpValue[][]): SExpValue[] => chain((x) => x, vals);
    return safe2((vars: VarDecl[][], vals: SExpValue[][]): Result<Value> => 
            bind(checkEqual(vars, vals), _ =>evalSequence(exp.body, makeExtEnv(GoodVars(vars), GoodVals(vals), env))))(Vars, tupleVals);
}
