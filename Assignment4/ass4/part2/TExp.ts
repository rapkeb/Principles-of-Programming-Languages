...
const parseCompoundTExp = (texps: Sexp[]): Result<CompoundTExp> => 
    ...
    parseLonelyTupleTExp(texps);

const parseLonelyTupleTExp = (texps: Sexp[]): Result<TupleTExp> =>
    bind(parseTupleTExp(texps),(tuple: TExp[]) =>
            isEmpty(tuple) ? makeOk(makeEmptyTupleTExp()) :
            allT(isNonTupleTExp , tuple) ? makeOk(makeNonEmptyTupleTExp(tuple)) :
            makeFailure(`Bad tuples - ${texps}`));

const parseProcTExp = (texps: Sexp[]): Result<ProcTExp> => {
    const pos = texps.indexOf('->');
    return (pos === -1)  ? makeFailure(`There is no -> - ${texps}`) :
           (pos === 0) ? makeFailure(`No parameters - ${texps}`) :
           (pos === texps.length - 1) ? makeFailure(`There is no type for return - ${texps}`) :
           (texps.slice(pos + 1).indexOf('->') > -1) ? makeFailure(`one -> only - ${texps}`) :
