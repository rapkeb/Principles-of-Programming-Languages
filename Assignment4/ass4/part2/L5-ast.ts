...
//         |  ( let-values ( <values>* ) <cexp>+ )  / LetValuesExp(bindings:Values[], body:CExp[]))
//         |  ( set! <var> <cexp>)            / SetExp(var: varRef, val: CExp)
// <binding>  ::= ( <var-decl> <cexp> )            / Binding(var:VarDecl, val:Cexp)
// <values>  ::= ( ( <var-decl>* ) <cexp> ) / Values(vars:VarDecl[], val:Cexp)
...
export interface Values {tag: "Values"; vars: VarDecl[]; val: CExp; }
export const makeValues = (vars: VarDecl[], val: CExp): Values =>
    ({tag: "Values", vars: vars, val: val});
export const isValues = (x: any): x is Values => x.tag === "Values";

export interface LetValuesExp {tag: "LetValuesExp"; bindings: Values[]; body: CExp[]; }
export const makeLetValuesExp = (bindings: Values[], body: CExp[]): LetValuesExp =>
...
const isGoodValues = (bindings: Sexp): bindings is [Sexp[], Sexp][] =>
...
const parseLetValues = (bindings: Sexp, body: Sexp[]): Result<LetValuesExp> =>
    isEmpty(body) ? makeFailure('Body cannot be empty') :
    ! isGoodValues(bindings) ? makeFailure(`Bad bindings: ${JSON.stringify(bindings)}`) :
    safe2((bindings1: Values[], body: CExp[]) => makeOk(makeLetValuesExp(bindings1, body)))
        (parseValues(bindings), mapResult(parseL5CExp, body));
...
const parseVarsDecl = (sexps: Sexp[]): Result<VarDecl[]> =>
...
const parseValues = (bindings: [Sexp[], Sexp][]): Result<Values[]> =>
    safe2((vars: VarDecl[][], vals: CExp[]) => makeOk(zipWith(makeValues, vars, vals)))
        (mapResult(parseVarsDecl, map(b => b[0], bindings)), mapResult(parseL5CExp, map(b => b[1], bindings)));
...
const unparseVarsDecl = (vars: VarDecl[]): Result<string> =>
    bind(mapResult(unparseVarDecl, vars), parsed => makeOk(`(${parsed.join(' ')})`));
...
const unparseValues = (bindings: Values[]): Result<string> =>
    bind(mapResult(bindings1 => safe2((varsdec: string, val: string) => makeOk(`(${varsdec} ${val})`))
        (unparseVarsDecl(bindings1.vars), unparse(bindings1.val)), bindings),
            (bindings2: string[]) => makeOk(join(" ", bindings2)));

const unparseLetValues = (letV: LetValuesExp) : Result<string> => 
    safe2((bindings: string, body: string) => makeOk(`(let-values (${bindings}) ${body})`))
        (unparseValues(letV.bindings), unparseLExps(letV.body));
