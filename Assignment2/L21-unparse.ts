...
export const unparseL21 = (exp: Exp | Program): Result<string> =>
    ...
    isForExp(exp) ? safe3((start: string, end: string, body: string) => makeOk(`(for ${exp.vars.var} ${start} ${end} ${body})`))
                    (unparseL21(exp.startValue), unparseL21(exp.endValue), unparseL21(exp.Body)) :
