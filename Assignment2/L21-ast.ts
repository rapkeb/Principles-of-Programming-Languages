...
export interface ForExp {tag: "ForExp"; vars: VarDecl; startValue: NumExp; endValue: NumExp; Body: CExp; };
...
export const makeForExp = (vars: VarDecl, startValue: NumExp, endValue: NumExp, Body: CExp): ForExp =>
    ({tag: "ForExp", vars: vars, startValue: startValue, endValue: endValue, Body: Body});
...
// <ForExp> -> (for <varDecl> <NumExp> <NumExp> <CExp>)
const parseForExp = (varName: Sexp, body: Sexp[]): Result<ForExp> =>
    isIdentifier(varName) && body.length == 3 ?
        bind(mapResult(parseL21CExp, body),
            (cexps: CExp[]) => isNumExp(cexps[0]) && isNumExp(cexps[1]) ?
            makeOk(makeForExp(makeVarDecl(varName), cexps[0], cexps[1], cexps[2])) :
            makeFailure("not NumExp")) :
