...
describe('L4 Normal Eval', () => {
    ...
    it('test 1', () => {
        expect(bind(parseL4(`(L4 (let ((a 5) (b #f)) (if b a (+ a 1))))`), evalNormalProgram)).to.deep.equal(makeOk(6));

        expect(bind(parseL4(`(L4 (let ((f (lambda (n) (if (= n 0) 1 (* n (f (- n 1))))))) (f 3)))`), evalNormalProgram)).to.deep.equal(makeOk(6));
    });
    ...
    it('test 2', () => {
        expect(bind(parseL4(`(L4 (define f (lambda (x y)
                    ...
                                    (let ((y (* x x))) y)
                                    (f (- x 1) (+ y 1)))))
                                (f 12 8))`), evalNormalProgram)).to.deep.equal(makeOk(100));
    });

    it('test 3 - captures variables in closures', () => {
        expect(bind(parseL4(`
            (L4 (define makeMultiple (lambda (n) (lambda (x) (* x n))))
                (define x1 (makeMultiple 6))
                (define x2 (makeMultiple 2))
                (+ (x1 1) (x2 2)))`), evalNormalProgram)).to.deep.equal(makeOk(10));
    });
});
